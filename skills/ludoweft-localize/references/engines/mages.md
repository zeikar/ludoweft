...
Several characters are control heads, and one that means itself is escaped with a backslash. Counting a whole shipped localization settles the set: in one game the escaped characters were `\%`, `\#`, `\$`, `\&` — 45, 19, 2 and 1 occurrences — with no unescaped literal anywhere. Every unescaped occurrence was structural: `%` heading a `%C`/`%p` control, `$` opening a `${name}` runtime variable. Those structural ones are protected tokens, so `validate` guards them; the escaped literals it cannot see at all.

Enumerate the set rather than meeting it one character at a time. Extract every backslash in the shipped localization together with the character after it and tally them — the whole escape vocabulary falls out in one pass, and the alternative is discovering each one from a shipped bug. Both escapes found in that project surfaced because a translator noticed the reference text was consistent about a character no rule mentioned.
